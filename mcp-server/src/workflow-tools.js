...
import {
  PROMPT_PACKET_KINDS,
  buildPromptPacket,
  configurePromptPackets,
  formatPromptPacket,
  latestExecutedVerification,
} from "./prompt-packets.js";
...
function getActiveMapSlug() {
  let value = "";
  try {
    value = fs.readFileSync(path.join(mapsDir(), "active"), "utf8").trim();
  } catch {
    return null;
  }
  if (value && fs.existsSync(mapPath(value))) {
    return value;
  }
  return null;
}

function loadMapRecord(name) {
  const raw = String(name || "").trim();
  const slug = raw ? findExistingSlugByName(raw, mapPath) : getActiveMapSlug();
  if (!slug) {
    return [null, null];
  }
  const record = readJsonRecover(mapPath(slug), () => null);
  if (!record || typeof record !== "object" || !Array.isArray(record.tickets)) {
    return [slug, null];
  }
  return [slug, record];
}
...
// Prompt packet builders live in prompt-packets.js and read durable state
// through the same dependency-injection shape used here.
...
function workflowRouteState() {
  ...
  const [activeMapSlug, activeMap] = loadMapRecord();
  ...
  if (activeMap && activeMap.status !== "promoted") {
    mapView = {
      id: activeMapSlug,
      destination: activeMap.destination || "",
      status: activeMap.status || "charting",
      open_tickets: mapOpenTickets(activeMap).length,
      frontier: mapFrontierTickets(activeMap).length,
      decisions: Array.isArray(activeMap.decisions) ? activeMap.decisions.length : 0,
      fog: mapUngraduatedFog(activeMap).length,
      clear: mapIsClear(activeMap),
    };
  }
...
function routeCommandFor(route, task, stateView, classification = {}) {
  ...
  if (route === "map") {
    const activeMap = stateView.active_map;
    if (activeMap) {
      return activeMap.clear ? "mythify map promote" : "mythify prompt map";
    }
    return `mythify map create ${quotedTask}`;
  }
  if (route === "research") {
...
function selectWorkflowRoute(task, stateView, classification) {
  ...
  if (routeHas(text, ROUTE_MAP_TERMS)) {
    return [
      "map",
  ...
  if (stateView.active_map && routeHas(text, ROUTE_RESUME_TERMS)) {
    return [
      "map",
...
export function registerWorkflowTools(server, nextDeps) {
  ...
  configurePromptPackets({
    ...
    getActiveResearchSlug,
    getActiveMapSlug,
    campaignNextAction,
    buildCampaignPromptPayload,
  });
  // ---------------------------------------------------------------------------
  // Classification tool
  // ---------------------------------------------------------------------------
