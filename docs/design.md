...
(current 1.x) and `zod` (4.x). package.json: name `mythify-mcp`, version `3.6.7`,
...
This is Mythify v3.6.7. Fanout was added in 2.1.0; 2.2.0 added local
...
captured subprocess output for local CLI and command fanout workers.
The CLI prints no version banner; the MCP server reports 3.6.7 through its
server info.
