...
| `prompt KIND [NAME] [--goal TEXT] [--verify COMMAND] [--json]` | Render a read-only workflow prompt packet. Kinds are `research`, `analysis`, `failure`, `handoff`, `review`, `campaign`, `map`, and `next`; packet output is steering material for the host, not verification evidence. The `review` packet carries a critic protocol for subjective quality: name a best-in-class reference, judge blind side by side, grade the integrated deliverable rather than intermediate artifacts, and record critic verdicts with `verify claim` while executed checks stay the completion gate. `next` selects failure recovery only when the latest executed check is red, then campaign, map, research, handoff, or analysis based on active state. | 0; 1 if no workspace or named state is missing |
...
| `plan_create` | `{goal: string, name?: string, steps?: [{title: string, success_criteria?: string, verify_command?: string, phase?: enum, vertical_slice?: object}], horizon?: number, archetype?: enum(direct, rpi, design-heavy), design?: string, parents?: [{kind: string, id: string}]}` | Ids auto-assigned 1-based and the plan becomes active. `horizon` creates default lookahead steps when `steps` is omitted. Design-heavy build steps require a vertical slice with an observable result and at least one automated or manual check. Typed parents capture current revisions. |
| `plan_add_step` | `{title: string, success_criteria?: string, verify_command?: string, phase?: enum, vertical_slice?: object, plan?: string}` | Append to the named or active plan. Explicit phases and vertical-slice validation match `plan_create`. |
