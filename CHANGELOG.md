...
- Extracted MCP plan tool registrations into the direct-import
  `mcp-server/src/plan-tools.js` module.
- Added direct module coverage for the plan tool registrar, including strict
  step evidence enforcement.

## [3.6.49] - 2026-06-16

### Changed

- Extracted MCP outcome loop tool registrations into the direct-import
  `mcp-server/src/outcome-tools.js` module.
- Added direct module coverage for the outcome tool registrar, including
  outcome iteration recording and the `MYTHIFY_DISABLE_RUN` refusal path.
...
- Extracted MCP read-only view tool registrations into the direct-import
  `mcp-server/src/view-tools.js` module.
- Added direct module coverage for the view tool registrar while keeping the
  surface manifest guard aware of extracted registration modules.
...
- Extracted Python memory and lesson stores plus their CLI command handlers into
  a direct-import `mythify_memory.py` helper module.
- Added direct module coverage for memory and lesson helpers while keeping
  copied drop-in, status, summary, and local eval coverage.
...
- Extracted MCP execution_probe and execution_run helpers into a direct-import
  `src/execution-adapter.js` module.
- Added direct module coverage for execution adapter guard behavior while
  keeping existing execution_probe and execution_run MCP tests.
...
- Classification policy facts now live in the shared
  `protocol/classification-rules.json` manifest, including thresholds, risk and
  ceremony buckets, fanout policy, execution profiles, model triage, next
  actions, and verification hints. The CLI and MCP server load the same policy
  data, and the manifest checker now validates those shared sections.
...
- Explicit `host_cli_probe` and `host_cli_run` `bin` paths now must use a
  basename that matches the selected host CLI family, so a remote tool caller
  cannot point the host-CLI adapter at an arbitrary executable name.
...
- CLI `report --mark --since ...` and MCP `work_report` with `mark` plus
  `since` now fail clearly instead of silently marking the cursor without
  showing events.
