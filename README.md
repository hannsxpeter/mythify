...
| MCP server | `mcp-server/` | Node 18+ server exposing the same state directory through 26 MCP tools, including task classification, host model switch state, provider probes, host CLI probes, execution probes, lifecycle probes, outcome loops, and parallel delegation (fanout). |
...
| `host-model status [--json]` | Show the recorded host model switch, if any. | 0; 1 if no workspace |
...
`allow_stronger`. This keeps cheap triage and mechanical fanout from
accidentally exceeding the orchestrating model. Unknown local CLI defaults are
recorded as `uncheckable`; Mythify does not guess what Codex Desktop, Claude,
or Cursor currently maps a blank model setting to.
...
```json
{
  ...
  "tasks": [
    {"title": "High-risk review", "prompt": "Review this patch for bugs.", "model": "sonnet"}
  ]
}
```
