...
| MCP server | `mcp-server/` | Node 18+ server exposing the same state directory through 32 MCP tools, including task classification, host model switch state, provider probes, local model runs, host CLI probes, bounded host CLI worker runs, execution probes and runs, lifecycle probes, outcome loops, workflow status, background task status, phase status, and parallel delegation (fanout). |
